...
### 2026-05-24 最终适配与重新提交
- [x] 增加数据库连接与初始化向导：未安装 PostgreSQL 时主窗口仍可打开，并提供官网下载链接、连接检测、本机配置保存、一键建库和执行内置 schema.sql。
- [x] 增加数据库离线浏览降级：数据库不可用时仍可浏览目录、缩略图、查看图片和幻灯片播放，数据库功能在界面上明确标注未连接。
- [x] 增加“补打 AI 标签”入口，解决自动识别遗漏、子目录刚同步或未刷新时的手动补救问题。
- [x] 设置窗口改为单实例，重复点击只置顶已有窗口，避免多个设置页互相覆盖配置。
- [x] 重新生成课程论文、附加说明、评分表、目标 JAR、源码 ZIP，并同步到第07组正确提交目录。
