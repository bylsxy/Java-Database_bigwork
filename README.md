...
2. 数据库系统基础部分重点体现 PostgreSQL 表设计、主外键约束、自引用目录树、多对多标签关系、`bytea` 缩略图、逻辑删除、操作日志、视图、索引、触发器、存储过程、递归 CTE、事务回滚、连接池和 PreparedStatement。
...
16. 右键菜单可以打开图片所在文件夹。
17. 删除图片前会进行二次确认。
18. 删除时会同步处理磁盘文件和数据库记录。
19. 数据库触发器会记录逻辑删除日志。
20. 复制图片时会记录复制操作。
21. 粘贴图片时会处理目标目录重名冲突。
22. 粘贴图片后会为新文件写入数据库记录。
23. 单张重命名会同步修改磁盘文件名和数据库文件名。
24. 批量重命名支持自定义前缀。
25. 批量重命名支持设置起始编号。
26. 批量重命名支持设置编号位数。
27. 批量重命名窗口提供新旧文件名预览。
28. 批量重命名使用事务处理数据库更新，失败时回滚已改动的文件名。
...
4. `images` 表保存图片元数据、缩略图、哈希、AI 处理状态和逻辑删除状态。
...
15. `v_active_images` 视图过滤逻辑删除图片并关联目录。
...
25. `trg_image_before_update` 自动记录重命名和逻辑删除日志，并更新时间戳。
