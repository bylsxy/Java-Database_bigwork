...
def make_report_docx():
    ...
    add_para(doc, "本次系统分析以课程题目要求和最终程序功能为依据。文档重点围绕目录扫描、图片预览、图片查看器、幻灯片播放、全局主题背景、数据库持久化和设置页等已经落地的内容展开，避免把尚未稳定进入主流程的设想写成已完成功能。")
    ...
    add_table(doc, [
        ...
        ("安全配置路径", "AI端点、AI密钥和数据库连接均写入本机外部配置，不把个人密码和密钥写死到源码。", "交付和验收时不泄露本机私密配置。"),
    ...
    add_para(doc, "小组分工按照开发过程和最终成果重新归纳。评分表中的工作量按小组协作口径保持基本均分，正文分工则按程序、数据库、文档和测试材料的实际完成内容写清楚。")
    add_table(doc, [
        ("毕振岚", "组长、主程序实现", "主导 JavaFX 主体编码、图片管理核心流程、界面修复和最终程序整合。", "主界面、目录树、缩略图、查看器、幻灯片和编辑器等核心界面均在当前程序中体现。"),
        ("陈厚华", "扩展与测试协助", "参与幻灯片音乐、外部接口可行性验证、运行流程检查和验收反馈。", "扩展功能和运行检查体现在幻灯片设置、设置页测试和完整流程测试中。"),
        ("徐阳", "文档与提交整理", "负责数据库设计文档、课程论文整合、提交目录整理和格式核查。", "数据库 schema、课程论文、评分表、附加说明和最终提交目录均按老师要求完成整理。"),
    ], headers=["成员", "主要定位", "实际承担内容", "成果体现"], widths=[2.4, 3.2, 5.2, 4.7])
...
""")
    ...
    add_para(doc, "我这次主要负责把项目材料整理成能交、能看、能对上老师要求的报告。前面的数据库需求分析、文档整理和后面的论文整合都很考验耐心：程序已经实现了什么、老师模板要求什么、提交目录需要哪些文件，都要放到同一个文档逻辑里。只靠自动生成一版报告是不够的，封面表格、测试表、分工、参考资料这些地方都要逐项核对。")
...
def make_score_docx():
    ...
    set_cell_text(table.cell(0, 8), CLASS_NAME, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    for p in doc.paragraphs:
        for run in p.runs:
            set_east_asia_font(run)
    out = STAGING / "2024级-软件工程-R5班第7组_评分表_电子图片管理程序.docx"
    doc.save(out)
    return out
...
def make_extra_docx():
    ...
    add_para(doc, f"班级：{CLASS_NAME}  第07组", 12, False, WD_ALIGN_PARAGRAPH.CENTER, False)
    ...
    add_table(doc, [
        ...
        ("2024级-软件工程-R5班第7组_评分表_电子图片管理程序.docx", "可编辑评分表，按电子图片管理程序评分表模板填写小组成员、任务分工、功能完成情况和代码质量。"),
